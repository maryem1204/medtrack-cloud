...
export class AppSidebarComponent {
  ...
  navItems: NavItem[] = [
    ...
    {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 3h18v4H3V3zm2 6h14v12H5V9zm3 3v6m4-6v6m4-6v6" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"></path></svg>`,
      name: "Ventes",
      path: "/ventes",
    },
    {
  ...
  private routesParRole: Record<string, string[]> = {
    ADMIN: ['/dashboard', '/appareils', '/fournisseurs', '/clients', '/commandes', '/maintenance', '/alertes', '/ventes', '/utilisateurs'],
    COMMERCIAL: [ '/appareils', '/fournisseurs', '/clients', '/commandes', '/ventes'],
    TECHNICIEN: [ '/appareils', '/maintenance', '/alertes'],
    CLIENT: [ '/appareils'],
  };

  get navItemsFiltres(): NavItem[] {
    const role = this.authService.getUser()?.role || '';
    const routesAutorisees = this.routesParRole[role] || [];
    return this.navItems.filter(item => {
      if (item.path) return routesAutorisees.includes(item.path);
      if (item.subItems) return item.subItems.some(s => routesAutorisees.includes(s.path));
      return true;
    });
  }
